...
dispatcher.onPost("/Solution", function(req, res) {
	...
	maindb.rquery("SELECT A.Answer, A.Next, A.Latitude, A.Longitude, B.HashID FROM Questions AS A INNER JOIN Questions AS B WHERE A.Id = ? AND A.HashID = ? AND A.Next = B.ID;", function (err, sqlres) {
		if(sqlres.length==1) {
...
dispatcher.onPost("/Challange", function(req, res) {
	...
	//res.write("b");
	//res.end();
	maindb.rquery("SELECT Question FROM Questions WHERE Id = ? AND HashID = ?;", function (err, sqlres) {
...
dispatcher.onPost("/Quest", function(req, res) {
	...
	res.writeHead(200, {'Content-Type': 'text/plain'});
	maindb.rquery("SELECT Start, Name, Description FROM Quests WHERE ? < Latitude AND Latitude < ? AND ? < Longitude AND Longitude < ?;", function (err, sqlres) {
		var resp = [];
